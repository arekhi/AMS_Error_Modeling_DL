    ...
    def apply_mask(self):
        for name, param in self.model.named_parameters():
            try:
                ...
                # The mixed-signal error wrapper adds a new term
                # If these are the sources of error, workaround and move on
                name_parts = name.split('.')
                name_in_dict = False
                if 'wrapped_module' in name_parts:
                    name_parts.remove('wrapped_module')
                    name_in_dict = True
