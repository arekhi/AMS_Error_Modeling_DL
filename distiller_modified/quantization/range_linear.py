...
Bmag_W = 7
Bmag_X = 7
B_lost = Bmag_W + Bmag_X + np.log2(N_mult) - B_ADC
...
class RangeLinearQuantWrapper(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        stddev = np.sqrt(N_tot/N_mult * 1/12) * 2**B_lost
        noiseTensor = torch.autograd.Variable(torch.zeros(accum.size()).cuda())
        accum.add_(noiseTensor.data.normal_(0,stddev))
