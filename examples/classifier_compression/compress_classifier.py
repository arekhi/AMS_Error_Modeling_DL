...
import pickle
import distiller.directives as directives
...
#parser.add_argument('--B_ADC', type=int, default=None, help='Precision of ADC (e.g. --B_ADC 9)')
#parser.add_argument('--N_mult', type=int, default=None, help='Number of multipliers per ADC (e.g. --N_mult 8)')
#parser.add_argument('--B_W', type=int, default=None, help='Precision of weights (e.g. --B_W 8)')
#parser.add_argument('--B_X', type=int, default=None, help='Precision of activations (e.g. --B_X 8)')
...
#        return evaluate_model(model, criterion, test_loader, pylogger, args)
    ...
    if args.freeze_bn:
        directives.freeze_training(model, 'bn', freeze='True')

    if args.freeze_conv:
        directives.freeze_training(model, 'conv', freeze='True')

    if args.freeze_fc:
        directives.freeze_training(model, 'fc', freeze='True')
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        apputils.save_checkpoint(epoch, args.arch, model, optimizer, compression_scheduler, best_top1, is_best, args.name, msglogger.logdir)
#        apputils.save_checkpoint(epoch, args.arch, model, optimizer, compression_scheduler, best_top1, is_best, 'epoch_'+str(epoch), msglogger.logdir)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1):
    ...
    for validation_step, (inputs, target) in enumerate(data_loader):
        with PytorchNoGrad():
            ...
            output = model(input_var)
            stddev = np.sqrt(2048/N_mult * 1/12) * 2**B_lost * 2**(-(B_W-1 + B_X-1))  # MIXED-SIGNAL ERROR MAGNITUDE CALCULATION
            output.add_(torch.cuda.FloatTensor(output.size()).normal_(0,stddev))      # MIXED-SIGNAL ERROR INJECTION
